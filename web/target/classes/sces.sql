...
alter table teacher_class
    add constraint teacher_class_ibfk_1
        foreign key (teacherId) references teacher (id);
...
create table class_item(
                           ...
                           foreign key (classId) references class(id),
                           foreign key (evaluationitemId) references evaluationitem(id)
)


select * from class where id in (select classId from teacher_class where teacherId = 5);

select * from teacher where userId = "admin";

drop table class_item;